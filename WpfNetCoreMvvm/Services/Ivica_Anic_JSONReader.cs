...
namespace ReaderSample
{
    class Ivica_Anic_JSONReader
    {
        ...
        public static async IAsyncEnumerable<string> ReadJSONFile()
        {
           ...
           string output1= await AsyncWebExample(@"http://universities.hipolabs.com/search?country=United%20States");
            yield return output1;
           
           string output2 = await AsyncWebExample(@"http://universities.hipolabs.com/search?", worldWide: true);
